...
The app is still a prototype shell. It should show a signed Markdown page, calm cached/offline status, and key-derived author/document addresses. You can add pages to your place, switch between them in the writer, preview Markdown before publishing, copy each page's stable document address or Markdown link, copy the current reader page or place as a Markdown link, delete non-home pages, reorder pages, save edits, publish signed records from the writer or page list while keeping the protected home page as the place entry point, visit a second sample author over localhost, take that sample author offline and keep reading its verified cached copy, visit pages from the current signed manifest, follow `fork://` links inside signed Markdown, switch between colorful reader-owned themes, visit cached Fork addresses, and save bookmarks with normalized local nicknames. The prototype serves and fetches signed record bundles over localhost, then rejects incomplete, forged, or internally inconsistent bundles before they enter the verified cache.
...
- browse without tabs using history, local nicknames, and visible place pages
- follow `fork://` links between signed Markdown pages
- render Markdown with colorful reader-owned themes
