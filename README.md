...
The app is still a prototype shell. It should show a signed Markdown page, the calm cached/offline status, and key-derived author/document addresses. You can create local drafts, switch between them in the writer, preview Markdown before publishing, copy each draft's stable document address, delete non-home drafts, save edits, publish signed records for the local draft list with the selected draft as home, visit a second sample author over localhost, take that sample author offline and keep reading its verified cached copy, visit pages from the current signed manifest, switch between simple reader themes, visit cached Fork addresses, and save bookmarks with local nicknames. The prototype serves and fetches signed record bundles over localhost.
